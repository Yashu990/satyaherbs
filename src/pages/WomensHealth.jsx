...
import heroImg from '../images /hero_womens_health.png'
  ...
  const products = [
    {
      ...
      rating: 4.9,
      reviews: 4127,
      description: 'Naturally regulate hormones, ease PMS symptoms, and restore complete hormonal harmony through every phase of life.',
      features: ['Hormonal Balance', 'PMS Relief', 'Mood Support', 'Natural Herbs'],
      badge: 'Best Seller',
      img: prod1
    },
    {
      id: 2,
      name: "Women's Beauty & Glow",
      price: '₹1,999',
      originalPrice: '₹2,699',
      rating: 4.8,
      reviews: 3568,
      description: 'Radiant skin, stronger nails, and lustrous hair from within with beauty-boosting minerals and adaptogens.',
      features: ['Glowing Skin', 'Hair Strength', 'Nail Health', 'Collagen Boost'],
      badge: 'Top Rated',
      img: prod2
    },
    {
      id: 3,
      name: "Women's Complete Wellness",
      price: '₹5,499',
      originalPrice: '₹7,999',
      rating: 4.9,
      reviews: 5201,
      description: 'All-in-one bundle covering hormonal health, energy, beauty, and bone care for total women\'s wellness.',
      features: ['4 Products', 'Bone Care', 'Energy Boost', 'Free Shipping'],
      badge: 'Best Value',
      img: prod3
    }
  ]
  const stats = [
    { value: '2L+', label: 'Women Empowered' },
    { value: '2 Wks', label: 'Visible Results' },
    { value: '0', label: 'Side Effects' },
    { value: '4.9★', label: 'Avg Rating' }
  ]
  const benefits = [
    { icon: '🌸', title: 'Hormonal Harmony', desc: 'Natural herbs to balance estrogen and progesterone' },
    { icon: '✨', title: 'Inner Beauty', desc: 'Glow from within with collagen-boosting minerals' },
    { icon: '🦴', title: 'Bone & Joint', desc: 'Essential minerals for lifelong bone strength' },
    { icon: '⚡', title: 'Energy & Vitality', desc: 'Combat fatigue with adaptogenic Shilajit extracts' }
  ]
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="relative w-full overflow-hidden" style={{ minHeight: '560px' }}>
        ...
        <div className="absolute inset-0" style={{ background: 'linear-gradient(90deg, rgba(0,0,0,0.88) 0%, rgba(0,0,0,0.6) 60%, rgba(0,0,0,0.2) 100%)' }}></div>
        <div className="relative z-10 flex flex-col justify-center px-8 md:px-20" style={{ paddingTop: '120px', paddingBottom: '60px' }}>
          <div className="max-w-2xl">
            <span className="inline-block px-4 py-1 rounded-full text-xs font-semibold tracking-widest uppercase mb-4" style={{ background: 'rgba(200,164,90,0.2)', color: '#c8a45a', border: '1px solid rgba(200,164,90,0.5)' }}>
              🌸 Loved by 2 Lakh+ Women
            </span>
            <h1 className="font-serif font-bold text-white mb-4" style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', lineHeight: '1.1' }}>
              Women's <span style={{ color: '#c8a45a' }}>Health</span>
            </h1>
            <p className="text-lg mb-8" style={{ color: '#e0e0e0', maxWidth: '520px' }}>
              Complete wellness for every woman — hormonal balance, beauty, energy, and vitality through every stage of life
            </p>
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="text-center mb-12">
          <h2 className="font-serif font-bold text-black mb-3" style={{ fontSize: '2rem' }}>Comprehensive Women's Wellness</h2>
          <p className="text-gray-600 max-w-2xl mx-auto">Specially formulated for women's unique health needs. Support hormonal balance, radiant beauty, and sustained energy.</p>
        </div>
          ...
          {products.map((product) => (
            <div key={product.id} className="rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 bg-white border border-gray-100">
              ...
              <div className="p-6">
                ...
                <button className="w-full py-3 rounded-xl font-bold text-white transition-all hover:opacity-90" style={{ background: 'linear-gradient(135deg, #0f0508, #1f1018)' }}>Add to Cart</button>
              </div>
            </div>
          ))}
        </div>
        <div className="mb-16">
          <h3 className="font-serif font-bold text-center text-black mb-8 text-2xl">Why Women Love SATYAHERBS?</h3>
        ...
        <div className="rounded-2xl p-10 text-center text-white relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0f0508, #1f1018)' }}>
          <h3 className="font-serif font-bold text-3xl mb-3">Embrace Your Best Self</h3>
          <p className="text-lg mb-6" style={{ color: '#d0d0d0' }}>200,000+ women trust SATYAHERBS. 100% natural, certified, results visible in just 2 weeks.</p>
          <button className="px-10 py-4 rounded-xl font-bold text-black text-lg transition-all hover:scale-105" style={{ background: 'linear-gradient(135deg, #c8a45a, #e8c87a)' }}>Shop Women's Health →</button>
        </div>
      </div>
    </div>
  )
